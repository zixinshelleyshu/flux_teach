...
In this chapter we will introduce a model which includes the time-invariant variables to compute the predictions. The time-invariant variables are usually measurement of every sites. We can train this model on the data from multiple sites and predict the GPP values on a new site. Thus, we evaluate it with the predictions of an unseen site in the example. Here is an overview of the "LSTM with condition" in which the MLP is slightly tuned for better predictions. The "+" in the diagram means concatenation. 
...
CONDITIONAL_FEATURES <- ncol(meta_multi_train)-1# the dimension of the condition feature is the number of column of meta-data after one-hot encoding
...
```{r}
# since we have data from multiple sites, we would like to pass the time depended data from every site separately through the model to learn. We wrap the data by site and put them together for training.
...
for (epoch in 1:n_epochs){
  ...
  for (i in 1:ncol(training_df)){
    ...
    # compute the loss and R2 in the training
    train_loss <- c(train_loss, loss$item())# extract the loss value as float
...
```{r}
plot_df<-data.frame(GPP_NT_VUT_REF=sensor_multi_test_norm$GPP_NT_VUT_REF,pred=cv_pred[[1]] %>% as.numeric())
plot_df$ind<-seq(1,nrow(plot_df))
ggplot(plot_df)+geom_line(aes(ind,GPP_NT_VUT_REF))+geom_line(aes(ind, pred),color="red")

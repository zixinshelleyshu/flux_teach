...
df_meta_dummy<-data.frame(get_dummies.(df_meta_five%>%select(-c("sitename","koeppen_code","plant_functional_type")))%>%select(-colnames(df_meta_five%>%select(-c("sitename","koeppen_code","plant_functional_type")))))
                                                                                                                             
df_meta_dummy$sitename<-df_meta_five$sitename

#select the site that we use for testing
ind_CH_Lae<-c(df_five$sitename=="CH-Lae")


#split the train set and test set
meta_five_train<-df_meta_dummy[!ind_CH_Lae,]
sensor_five_train<-df_sensor_five[!ind_CH_Lae,]


meta_five_test<-df_meta_dummy[ind_CH_Lae,]
sensor_five_test<-df_sensor_five[ind_CH_Lae,]
...
CONDITIONAL_FEATURES <- ncol(meta_five_train)-1# the dimension of the condition feature is the number of column of meta-data after one-hot encoding
...
```{r, message=FALSE}
plot_df<-data.frame(GPP_NT_VUT_REF=sensor_five_test_norm$GPP_NT_VUT_REF,pred=cv_pred[[1]] %>% as.numeric())

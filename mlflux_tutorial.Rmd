...
```{r}
selected_five<-unique(df_five$sitename)
df_withind<-c()
for(i in 1:length(selected_five)){
   i_df<-df_five[df_five$sitename==selected_five[i],]
...
```{r}
ind_IT_lav<-df_five$sitename=="IT-Lav"
...
mymodule_generator_withcondition <- nn_module(
  ...
  initialize = function(input_dim, 
                        conditional_dim,# we need to have 
                        hidden_dim, 
                        conditional=1, 
                        num_layers = 1 
                        ){
...
CONDITIONAL_FEATURES <- ncol(df_meta_dummy)
conditional <- 1
```
...
for (epoch in 1:n_epochs){
    ...
    # Since "torch" operates on the data type "tensor" and the function only accept R atomic vector. So we transform the dataframe to matrix then to tensor data type 
    x <- torch_tensor(sensor_five_train_norm%>% 
                        select(-GPP_NT_VUT_REF)%>% 
                        as.matrix(), 
                      dtype = torch_float()
                      )$to(device = DEVICE)
    
    y <- torch_tensor(sensor_five_train_norm%>% 
                        select(GPP_NT_VUT_REF) %>% 
                        as.matrix(), 
                      dtype = torch_float()
                      )$to(device = DEVICE)
    
    c <- torch_tensor(as.matrix(meta_five_train), 
                       dtype = torch_float()
                       )$to(device = DEVICE)
...
```{r}
#cv_pred
#sensor_five_test_norm$GPP_NT_VUT_REF
